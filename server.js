...
// Scheduled task to update past appointments to 'completed' and insert records if not exist
cron.schedule('* * * * *', async () => {
...
// Run every hour to clear expired password reset tokens
cron.schedule('0 * * * *', async () => { // every hour at minute 0
  try {
    ...
    `);
    console.log(`Reset tokens before cleanup: ${countBefore.rows[0].count}`);

    // Log which tokens are expired
    const expiredTokens = await pool.query(`
      SELECT idusers, email, reset_token, reset_token_expiry
      FROM users
      WHERE reset_token IS NOT NULL AND reset_token_expiry <= NOW() AT TIME ZONE 'UTC'
    `);
    if (expiredTokens.rows.length > 0) {
      console.log('Expired tokens to be cleared:', expiredTokens.rows);
    } else {
      console.log('No expired tokens to clear at this time.');
    }
    ...
    const result = await pool.query(`
      ...
      WHERE reset_token IS NOT NULL AND reset_token_expiry <= NOW() AT TIME ZONE 'UTC'
    `);
    console.log(`Expired password reset tokens cleared. Rows affected: ${result.rowCount}`);

    // Count tokens after cleanup
    const countAfter = await pool.query(`
      SELECT COUNT(*) FROM users WHERE reset_token IS NOT NULL
    `);
    console.log(`Reset tokens after cleanup: ${countAfter.rows[0].count}`);
  } catch (err) {
